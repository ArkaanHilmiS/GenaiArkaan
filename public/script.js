...
// ============================================================
// APP SCRIPT — TernakBot
// ============================================================

marked.setOptions({ breaks: true, gfm: true });

// ===== SESSION =====
let sessionId = sessionStorage.getItem('ternakSessionId');
if (!sessionId) {
  sessionId = 'ternak-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
  sessionStorage.setItem('ternakSessionId', sessionId);
}
...
clearBtn.addEventListener('click', async () => {
  try { await fetch('/chat/' + sessionId, { method: 'DELETE' }); } catch {}

  sessionId = 'ternak-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
  sessionStorage.setItem('ternakSessionId', sessionId);
  totalTokens = 0;
  tokenText.textContent = '0 token';

  chatBox.innerHTML = `
    <div class="welcome-card" id="welcome-card">
      <div class="welcome-emoji">🐔</div>
      <h2>Percakapan Baru!</h2>
      <p>Riwayat dihapus. Saya siap membantu lagi, Peternak! 🌾</p>
    </div>`;
});
...
chatForm.addEventListener('submit', async e => {
  ...
  try {
    ...
    if (!res.ok) { appendMsg('bot', '⚠️ ' + (data.error || 'Terjadi kesalahan.')); return; }

    appendMsg('bot', data.message);
    if (data.metadata?.totalTokenCount) {
      totalTokens += data.metadata.totalTokenCount;
      tokenText.textContent = totalTokens.toLocaleString('id-ID') + ' token';
    }
    sessionLabel.textContent = `${data.historyLength || 0} pesan`;

  } catch (err) {
    thinking.remove();
    appendMsg('bot', '❌ Gagal terhubung ke server.');
  } finally {
...
function appendMsg(sender, text) {
  ...
  const content = sender === 'bot' ? marked.parse(text) : escapeHTML(text);
  row.innerHTML = `${avatar}<div><div class="msg-bubble">${content}</div><div class="msg-time">${time}</div></div>`;
...
function appendThinking() {
  ...
  row.innerHTML = `<div class="msg-avatar bot-av">🐔</div><div><div class="msg-bubble"><div class="thinking-dots"><span></span><span></span><span></span></div></div></div>`;
  chatBox.appendChild(row);
  chatBox.scrollTop = chatBox.scrollHeight;
  return row;
}
...
// ===== KALKULATOR FCR =====
document.getElementById('btn-hitung-fcr').addEventListener('click', async () => {
  const pakan    = parseFloat(document.getElementById('fcr-pakan').value);
  const telur    = parseFloat(document.getElementById('fcr-telur').value);
  const ayam     = document.getElementById('fcr-ayam').value;
  const periode  = document.getElementById('fcr-periode').value;

  if (!pakan || !telur) { alert('Isi total pakan dan total telur terlebih dahulu!'); return; }

  const btn = document.getElementById('btn-hitung-fcr');
  btn.textContent = '⏳ Menghitung...';
  btn.disabled = true;

  try {
    const res = await fetch('/hitung/fcr', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ totalPakan: pakan, totalTelurKg: telur, jumlahAyam: ayam || null, periode: periode || null })
    });
    const data = await res.json();

    document.getElementById('fcr-result').classList.remove('hidden');
    document.getElementById('fcr-nilai').textContent = data.fcr;
    document.getElementById('fcr-status').textContent = data.status;
    document.getElementById('fcr-pakan-per-ekor').textContent =
      data.pakanPerEkorKg ? `Konsumsi pakan per ekor: ${data.pakanPerEkorKg} kg` : '';

    // Scale marker (FCR 1.5 = 0%, FCR 3.5+ = 100%)
    const pct = Math.min(100, Math.max(0, ((data.fcr - 1.5) / 2) * 100));
    document.getElementById('fcr-marker').style.left = pct + '%';

    const analisisBox = document.getElementById('fcr-analisis');
    const analisisText = document.getElementById('fcr-analisis-text');
    if (data.analisis) {
      
      analisisText.innerHTML = marked.parse(data.analisis);
    }
  } catch {
    alert('Gagal menghitung. Pastikan server berjalan.');
  } finally {
    btn.textContent = '🧮 Hitung FCR';
    btn.disabled = false;
  }
});

// ===== KALKULATOR PRODUKSI =====
document.getElementById('btn-hitung-produksi').addEventListener('click', async () => {
  const telur   = document.getElementById('prod-telur').value;
  const ayam    = document.getElementById('prod-ayam').value;
  const harga   = document.getElementById('prod-harga').value;
  const pakan   = document.getElementById('prod-pakan-biaya').value;
  const ops     = document.getElementById('prod-ops').value;

  if (!telur || !ayam) { alert('Isi jumlah telur dan jumlah ayam terlebih dahulu!'); return; }

  const btn = document.getElementById('btn-hitung-produksi');
  btn.textContent = '⏳ Menghitung...';
  btn.disabled = true;

  try {
    const res = await fetch('/hitung/produksi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jumlahTelur: telur, jumlahAyam: ayam, hargaJual: harga || null, biayaPakan: pakan || null, biayaOperasional: ops || null })
    });
    const data = await res.json();

    document.getElementById('produksi-result').classList.remove('hidden');
    document.getElementById('hdp-nilai').textContent = data.hdp + '%';
    document.getElementById('hdp-status').textContent = data.statusHdp;

    if (data.keuangan) {
      document.getElementById('keuangan-grid').classList.remove('hidden');
      document.getElementById('k-pendapatan').textContent = 'Rp ' + data.keuangan.pendapatan;
      document.getElementById('k-biaya').textContent = 'Rp ' + data.keuangan.totalBiaya;
      document.getElementById('k-laba').textContent = 'Rp ' + data.keuangan.laba;
      document.getElementById('k-status-laba').textContent = data.keuangan.statusLaba;
    }
  } catch {
    alert('Gagal menghitung. Pastikan server berjalan.');
  } finally {
    btn.textContent = '🧮 Hitung Produksi';
    btn.disabled = false;
  }
});
...
fotoInput.addEventListener('change', e => {
  ...
  reader.onload = ev => {
    ...
    document.getElementById('foto-result').classList.add('hidden');
  };
  reader.readAsDataURL(file);
});

document.getElementById('btn-analisa-foto').addEventListener('click', async () => {
  const file = fotoInput.files[0];
  if (!file) { alert('Pilih foto terlebih dahulu!'); return; }

  const question = document.getElementById('foto-question').value || '';
  const btn = document.getElementById('btn-analisa-foto');
  btn.textContent = '⏳ Menganalisa...';
  btn.disabled = true;

  const formData = new FormData();
  formData.append('image', file);
  formData.append('message', question || 'Analisa gambar ini dari perspektif peternak ayam petelur. Berikan observasi dan rekomendasi yang berguna.');
  ...
  try {
    ...
    const resultBox = document.getElementById('foto-result');
    const resultText = document.getElementById('foto-result-text');
    resultBox.classList.remove('hidden');
    resultText.innerHTML = data.message ? marked.parse(data.message) : 'Tidak ada respons.';
  } catch {
    alert('Gagal menganalisa foto. Pastikan server berjalan.');
  } finally {
    btn.textContent = '🔍 Analisa dengan AI';
    btn.disabled = false;
  }
});